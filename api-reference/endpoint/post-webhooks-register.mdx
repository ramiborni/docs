...
Register a webhook URL to receive real-time event notifications from SendPilot. Webhooks are account-based (per user) and support both Clerk authentication (Bearer token) and API Key authentication.
...
```bash
curl -X POST https://integration-api-gateway.agreeablesea-e4815929.swedencentral.azurecontainerapps.io/api/account/webhooks/register \
